...
function LoginContent() {
  ...
  // default state for sign up page
  ...
  return (
    <div className={`w-full min-h-screen flex ${isDark ? 'bg-black' : ''}`}>
      ...
      <div
        className={`hidden md:flex md:w-1/2 relative ${isDark ? 'bg-zinc-900' : 'bg-blue-50'}`}
      >
      ...
      <div
        className={`w-full md:w-1/2 flex flex-col ${isDark ? 'bg-zinc-900 text-white' : 'bg-white'}`}
      >
        {/* Mobile only logo */}
        <div className="md:hidden flex items-center justify-center mt-6 mb-4">
          ...
          <span
            className={`font-sans text-sm font-extrabold ml-2 ${isDark ? 'text-white' : 'text-gray-600'}`}
          >
            hacktbd
          </span>
        </div>
        ...
        <div className="flex-1 flex flex-col justify-center px-6 sm:px-12 md:px-16 lg:px-24">
          ...
          {/* Condition to render Sign Up or Sign In form */}
          {activeTab === 'signup' ? (
            /* Sign Up Form */
            <form 
              className="space-y-6"
              onSubmit={async e => {
                ...
                setError('');
                try{
                  // Create Sign Up recrod (first factor)
                  await signUp.create({
                    firstName,
                    lastName,
                    emailAddress: email,
                  ...
                  // If require email verification, redirect to verify email page
                  if (signUp.status === 'missing_requirements') {
                    // trigger Clerk to send code/link to email
                    await signUp.prepareEmailAddressVerification();
                  } 
                  // If no verification is required, redirect to the next step
                  if (signUp.status === 'complete') {
                    await setActive({ session: signUp.createdSessionId });
                    // Clerk sets cookies and <SignIn> context flips
                  }
                } catch (err: any) {
            ...
            >
             {error && <p className='text-red-500'>{error}</p>} 
              ...
              <div className="relative flex items-center justify-center mt-6">
                <div
                  className={`flex-grow border-t ${isDark ? 'border-gray-700' : 'border-gray-300'}`}
                ></div>
                <span
