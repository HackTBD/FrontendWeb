 ...
 */
export default function HackathonDetailTeam({
  hackathon,
  isDark,
}: HackathonDetailTeamProps) {
  ...
  const calculateProgress = () => {
    ...
    return Math.min(
      (hackathon.currentParticipants / hackathon.maxParticipants) * 100,
      100
    );
  };
  ...
  return (
    ...
    >
      {/* Header */}
      <div
        className={`p-6 border-b ${isDark ? 'border-zinc-700' : 'border-gray-100'}`}
      >
        <h2
      ...
      <div className="p-6 space-y-6">
        ...
        <div>
          <h3
            className={`text-sm uppercase mb-1.5 ${isDark ? 'text-zinc-400' : 'text-gray-500'}`}
          >
            Team Size
          </h3>
          <div className="flex items-center">
            <UsersIcon
              className={`w-5 h-5 mr-2 ${isDark ? 'text-zinc-300' : 'text-gray-600'}`}
            />
            <p
        ...
        {/* Participants */}
        {hackathon.currentParticipants !== undefined &&
          hackathon.maxParticipants !== undefined && (
            <div>
              <div className="flex justify-between mb-1.5">
                <h3
                  className={`text-sm uppercase ${isDark ? 'text-zinc-400' : 'text-gray-500'}`}
                >
                  Participants
                </h3>
                <span
              ...
              {/* Progress bar */}
              <div
                className={`w-full h-2 rounded-full overflow-hidden ${isDark ? 'bg-zinc-700' : 'bg-gray-200'}`}
              >
                <div
        ...
        {hackathon.teamFormationEnabled && (
          <div
            className={`p-4 rounded-lg ${isDark ? 'bg-zinc-700/50' : 'bg-[#036CA0]/5'}`}
          >
            ...
            <p
              className={`text-sm mb-4 ${isDark ? 'text-zinc-300' : 'text-gray-600'}`}
            >
              Looking for teammates? Use our team formation tool to find the
              perfect match for your hackathon team.
            </p>
            <Button
