...
export default function HackathonTeams({
  ...
  function startMatching(teamId: string, backgroundNeeded: string, skillsNeeded: string) {
    ...
    .then(response => {
      if (!response.ok) {
        if (response.status === 404) {
          throw new Error('Matching API endpoint not found. Check that your Django server is running.');
        }
  ...
  return (
    <div className={`flex min-h-screen ${isDark ? 'bg-zinc-900' : 'bg-gray-100'}`}>
      <Sidebar activePath="/hackathons" hideLogo />
      <div className="flex-1 flex flex-col">
        <Header />
        <main className="flex-1 max-w-6xl mx-auto w-full py-10 px-4 sm:px-6 lg:px-8">
          <div className={`mb-8 border-b ${isDark ? 'border-zinc-700' : 'border-gray-200'} pb-5`}>
            ...
            <p className={`mt-2 text-lg ${isDark ? 'text-zinc-400' : 'text-gray-600'}`}>
          ...
          {!loading && !error && (!hackathonTeams || hackathonTeams.length === 0) && (
            <div className={`text-center py-16 ${isDark ? 'bg-zinc-800' : 'bg-white'} rounded-lg shadow-sm`}>
              <svg 
              ...
              <p className={`mt-4 text-lg font-medium ${isDark ? 'text-zinc-400' : 'text-gray-500'}`}>
                No teams found for this hackathon.
              </p>
              <p className={`mt-2 ${isDark ? 'text-zinc-500' : 'text-gray-400'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {hackathonTeams.map((team: any) => (
                ...
                >
                  ...
                  <div className={`space-y-3 ${isDark ? 'text-zinc-300' : 'text-gray-600'}`}>
                    <div className="flex items-start">
                      ...
                      <span>Created: <time dateTime={team.createdAt}>{new Date(team.createdAt).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })}</time></span>
                    </div>
                      ...
                      <div>
                        ...
                        <div className="mt-1 flex flex-wrap gap-1">
                          {(Array.isArray(team.skillsNeeded)
                            ? team.skillsNeeded
                            : typeof team.skillsNeeded === 'string' && team.skillsNeeded.trim()
                          ...
                          {(Array.isArray(team.skillsNeeded) && team.skillsNeeded.length === 0) ||
                            (typeof team.skillsNeeded === 'string' && !team.skillsNeeded.trim()) && (
                              <span className="text-sm italic">None specified</span>
                            )}
                        </div>
                      ...
                      <div>
                        ...
                        <div className="mt-1 flex flex-wrap gap-1">
                          {(Array.isArray(team.backgroundNeeded)
                            ...
                            : typeof team.backgroundNeeded === 'string' && team.backgroundNeeded.trim()
                          ...
                          {(Array.isArray(team.backgroundNeeded) && team.backgroundNeeded.length === 0) ||
                            (typeof team.backgroundNeeded === 'string' && !team.backgroundNeeded.trim()) && (
                              <span className="text-sm italic">None specified</span>
                            )}
                        </div>
                    ...
                    <div className="pt-4 mt-2 border-t border-dashed flex justify-end">
                      <button
                        ...
                        disabled={team.status !== 'OPEN' && team.status !== 'FORMING'}
                        onClick={() => {
                          if (team.status === 'OPEN' || team.status === 'FORMING') {
                            // Get the background and skills needed
                            const backgroundNeeded = Array.isArray(team.backgroundNeeded) 
                              ? team.backgroundNeeded.join(',') 
                              : typeof team.backgroundNeeded === 'string' 
                                ? team.backgroundNeeded 
                                : '';
                                
                            const skillsNeeded = Array.isArray(team.skillsNeeded) 
                            ...
                            startMatching(team.teamId || team.id, backgroundNeeded, skillsNeeded);
                          }
                        }}
                      >
                        <svg 
